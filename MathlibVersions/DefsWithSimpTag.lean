...
/-
We now want to prove that the tail-recursive definition agree with the original one: `∀ L : List R, SumSqTR L = SumSq L`. The key is that, when `S = SumSq L2`, the term  `SumSqAux S L1` can be computed in terms of `SumSq L1` and `SumSq L2`.
-/
theorem SumSqAuxWithSumSq [AddCommMonoid R] [Pow R ℕ] (L1 : List R) : ∀ L2 : List R, SumSqAux (SumSq L2) L1  = SumSq L2 + SumSq L1 := by
  induction L1 with
  | nil => simp only [SumSqAux, SumSq]; intro L2; simp only [add_zero]
...
lemma SumSqAuxEmptyList [AddCommMonoid R] [Pow R ℕ] (L : List R) : SumSqAux (SumSq []) L= SumSqAux (SumSq L) [] := by simp only [SumSqAuxWithSumSq]; simp only [SumSq, zero_add, add_zero]

theorem def_TR_ok [AddCommMonoid R] [Pow R ℕ] (L : List R) : SumSqTR L = SumSq L := by
  simp only [SumSqTR, SumSqAuxZero, SumSqAuxEmptyList, SumSqAux]
...
theorem squaring_and_summing [AddCommMonoid R] [Pow R ℕ] (L : List R) : SumSq L = (L.map (. ^ 2)).sum := by
  induction L with
  | nil => rfl
  | cons a l ih => simp only [SumSq, List.map, List.sum_cons, ih]
